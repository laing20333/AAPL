...
def k_prototype(n_clusters, max_iters, df):
    ...
    categorical_attributes_weight = [get_categorical_attributes_weight(df)] * n_clusters      # categorical data's weight

    # select k initial centroids
    centroid_id = random.sample(range(0, df.shape[0]), n_clusters)
    ...
    for it in xrange(0, max_iters):
        ...
        for object_id in xrange(0, df.shape[0]):
            ...
            for centroid_id, centroid_instance in enumerate(centroid):
                ...
                tmp_distance = kp_distance(x, y, categorical_attributes_weight[centroid_id])
        ...
        for cid in xrange(0, n_clusters):
            # if a cluster is empty, borrow a object from the other cluster
            if len(cluster_id[cid]) <= 0:
                cluster_id[cid].append(cluster_id[ (cid + 1) % n_clusters][0])
                cluster_id[ (cid + 1) % n_clusters].pop(0)
    ...
    # split original dataframe into n_clusters dataframe
    for c in xrange(0, n_clusters):
...
def modified_k_means_clustering(n_clusters, max_iters, df):
    ''' modified_k_mean_clustersing
        input:   1. the number of cluster
                    2. maximum iteration run into modified_k_mean_clustersing
                    3. data in pandas dataframe format
        output: 1. k cluster in dataframe list (each dataframe represents a cluster)
...
def evaluate_clustering(df_list, normalized_df_list, alg_name):
    ...
    for cid, df in enumerate(normalized_df_list):
        ...
        for object_idx in xrange(0, df.shape[0]):
            ...
            if alg_name == 'k_prototype':
                tmp_distance = kp_distance(x, y, categorical_attributes_weight[cid])
